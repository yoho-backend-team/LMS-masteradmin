...
import PlanView from "@/components/Subscription/PlanDetails"
...
export default function Subscription() {
  ...
  // Show plan view if a plan is selected for viewing
  if (viewingPlan) {
    return <PlanView plan={viewingPlan} onBack={handleBackToList} />
  }
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {plans.map((plan, idx) => (
          <Card key={idx} className="bg-white hover:bg-[#68b39f] transition-colors duration-300 group">
            ...
            <CardContent className="p-4 text-sm space-y-4">
              ...
              <p className="opacity-80 mb-4 group-hover:text-white" style={{ ...FONTS.text1 }}>
                {plan.description}
              </p>
              <div className="group-hover:text-white text-center" style={{ ...FONTS.number }}>
                {plan.price}
                <span className="text-sm font-normal ml-1" style={{ ...FONTS.text1, fontSize: "16px" }}>
                  / Monthly
                </span>
              </div>
              {(idx === 0 || idx === 1) && (
                    ...
                    {plan.features.map((feature, i) => (
                      <li key={i} className="flex items-center gap-2">
                        <span style={{ ...FONTS.text1 }}>
                          <img src={blacktick || "/placeholder.svg"} className="block group-hover:hidden w-4 h-4" />
                          <img src={tickicon || "/placeholder.svg"} className="hidden group-hover:block w-4 h-4" />
                        </span>
                        {feature}
                      </li>
                    ))}
