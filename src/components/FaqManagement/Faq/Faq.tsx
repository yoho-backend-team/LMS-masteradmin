...
const FAQ = () => {
  ...
  const handleAddFAQ = () => {
    if (!formData.question.trim() || !formData.category.trim()) return;
    setFaqs((prev) => [...prev, { id: Date.now(), ...formData }]);
    setIsAddOpen(false);
    setFormData({ question: "", description: "", category: "", status: true });
  };

  const handleEditFAQ = () => {
    setFaqs((prev) =>
      prev.map((faq) =>
        faq.id === editFaq.id ? { ...editFaq, ...formData } : faq
      )
    );
    setIsEditOpen(false);
    setEditFaq(null);
  ...
  <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50">
    <div className="bg-white p-6 rounded-xl w-[28%] h-2/3">
      <div className="flex">
        ...
        <div className="ml-auto cursor-pointer" onClick={() => setIsAddOpen(false)}>
      ...
      <div className="relative">
        <div
          className="border border-gray-300 rounded-lg px-3 py-2 w-full cursor-pointer"
          onClick={() => setIsOpen(!isOpen)}
        >
          { "Select a category"}
        </div>
        {isOpen && (
          <div className="absolute w-full bg-white shadow-lg mt-1 rounded-lg border border-gray-300 z-10 space-y-2 p-2">
            {categories.map((cat, index) => (
              <div
                ...
                className="px-3 py-2 border rounded-tl-xl hover:bg-[#68B39F] rounded-br-xl border-gray-200 hover:bg-gray-100 cursor-pointer"
              >
                {cat}
              </div>
            ))}
